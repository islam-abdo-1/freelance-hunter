...
class TestUtils:
    ...
    def test_estimate_difficulty(self):
        ...
        assert estimate_difficulty("Excel Formatting", "Format spreadsheets", ["excel"]) == "medium"
    
    def test_estimate_effort(self):
        """Test effort estimation."""
        budget = {"budget_min": 100}
        assert estimate_effort("Quick Task", "Small job", budget, "2 hours") == "low"
        assert estimate_effort("Large Project", "Comprehensive work", budget, "2 weeks") == "high"
    ...
    def test_validate_job_url(self):
        ...
        assert validate_job_url("https://example.com/random", "upwork") is False
...
class TestAgents:
    ...
    async def test_verification_agent(self):
        ...
        assert result.data["results"][0]["verification_status"] in ["VERIFIED", "PARTIALLY_VERIFIED"]
        ...
        assert result.data["results"][0]["verification_status"] in ["UNVERIFIED", "FAILED"]
    ...
    async def test_matching_agent(self):
        ...
        assert match["match_level"] in ["EXCELLENT_MATCH", "GOOD_MATCH"]
    ...
    async def test_proposal_agent(self):
        ...
        assert len(proposal["short"]) > 50
        assert len(proposal["normal"]) > 100
